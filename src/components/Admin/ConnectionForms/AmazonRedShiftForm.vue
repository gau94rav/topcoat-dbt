...
<script>
export default {
    props: ['loading', 'amazonRedShiftConnection', 'errors'],
    methods: {
        checkValidation(event) {
            this.$emit('checkValidation', event);
        },
        hasError(property) {
            return this.errors['amazonRedShiftConnection'] && this.errors['amazonRedShiftConnection'][property];
        }
    },
    computed: {
        connection: {
            get() {
                return this.amazonRedShiftConnection;
            },
            set(object) {
